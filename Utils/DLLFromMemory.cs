...
public unsafe class DLLFromMemory : IDisposable {
    ...
    private IntPtr pCode = IntPtr.Zero;
    ...
    private static void CopySections(ref IMAGE_NT_HEADERS OrgNTHeaders, IntPtr pCode, IntPtr pNTHeaders, byte[] data) {
    ...
    private static bool PerformBaseRelocation(ref IMAGE_NT_HEADERS OrgNTHeaders, IntPtr pCode, IntPtr delta) {
    ...
    private static IntPtr[] BuildImportTable(ref IMAGE_NT_HEADERS OrgNTHeaders, IntPtr pCode) {
    ...
    private static void FinalizeSections(ref IMAGE_NT_HEADERS OrgNTHeaders, IntPtr pCode, IntPtr pNTHeaders, uint PageSize) {
    ...
    private static void ExecuteTLS(ref IMAGE_NT_HEADERS OrgNTHeaders, IntPtr pCode, IntPtr pNTHeaders) {
    ...
    private static uint GetMachineType() => IntPtr.Size == 8 ? Win.IMAGE_FILE_MACHINE_AMD64 : Win.IMAGE_FILE_MACHINE_I386;

    private static uint AlignValueUp(uint value, uint alignment) => (value + alignment - 1) & ~(alignment - 1);

    private static IntPtr GetRealSectionSize(ref IMAGE_SECTION_HEADER Section, ref IMAGE_NT_HEADERS NTHeaders) {
    ...
    private static T PtrRead<T>(IntPtr ptr) where T : notnull => (T)Marshal.PtrToStructure(ptr, typeof(T))!;

    private static void PtrWrite<T>(IntPtr ptr, T val) where T : notnull => Marshal.StructureToPtr(val, ptr, false);

    private static IntPtr PtrAdd(IntPtr p, int v) => (IntPtr)(p.ToInt64() + v);

    private static IntPtr PtrAdd(IntPtr p, uint v) => IntPtr.Size == 8 ? (IntPtr)(p.ToInt64() + unchecked(v)) : (IntPtr)p.ToInt32() + unchecked((int)v);

    private static IntPtr PtrAdd(IntPtr p, IntPtr v) => IntPtr.Size == 8 ? (IntPtr)(p.ToInt64() + v.ToInt64()) : (IntPtr)p.ToInt32() + v.ToInt32();

    private static IntPtr PtrSub(IntPtr p, IntPtr v) => IntPtr.Size == 8 ? (IntPtr)(p.ToInt64() - v.ToInt64()) : (IntPtr)p.ToInt32() - v.ToInt32();

    private static IntPtr PtrBitOr(IntPtr p, UIntPtr v) => IntPtr.Size == 8 ? (IntPtr)unchecked((long)(unchecked((ulong)p.ToInt64()) | v.ToUInt64())) : unchecked((IntPtr)(int)(unchecked((uint)p.ToInt32()) | v.ToUInt32()));

    private static IntPtr PtrAlignDown(IntPtr p, UIntPtr align) => (IntPtr)unchecked((long)(unchecked((ulong)p.ToInt64()) & ~(align.ToUInt64() - 1)));

    private static bool PtrIsInvalidHandle(IntPtr h) {
        return h == IntPtr.Zero || h == (IntPtr.Size == 8 ? (IntPtr)(long)-1 : (IntPtr)(-1));
    }

    private static bool PtrSpanBoundary(IntPtr p, uint Size, int BoundaryBits) => unchecked((ulong)p.ToInt64()) >> BoundaryBits < (unchecked((ulong)p.ToInt64()) + Size) >> BoundaryBits;

    private static T BytesReadStructAt<T>(byte[] buf, int offset) where T : notnull {
